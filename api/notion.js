...
const DEFAULT_SETTINGS = {
  ...
  brief_labels:        { value: 'Db/Jobs/Shifts, Db/Banking', about: 'Gmail labels the brief still reads even after sorting archives them, comma separated — exact label names as they appear in Gmail' }
};
...
/*
 * Same running-total row the engine's own trackSpend() writes to
 * (ai_spend_month in Settings) — reused here rather than starting a second,
 * separate counter, since a follow-up draft is genuinely the same kind of
 * spend as a brief-writing call, just triggered from the app instead of a
 * scheduled trigger.
 */
async function trackAppSpend(cost) {
  if (!cost) return null;
  const monthKey = new Date().toISOString().slice(0, 7);
  try {
    const r = await notion(`databases/${DB.settings}/query`, 'POST', {
      filter: { property: 'Name', title: { equals: 'ai_spend_month' } }, page_size: 1
    });
    let total = cost;
    if (r.results.length) {
      const p = r.results[0];
      const storedMonth = txt(p.properties.Value);
      const storedTotal = (p.properties['Number value'] || {}).number || 0;
      total = (storedMonth === monthKey ? storedTotal : 0) + cost;
      await notion('pages/' + p.id, 'PATCH', { properties: {
        Value: { rich_text: [{ text: { content: monthKey } }] },
        'Number value': { number: total }
      }});
    } else {
      await notion('pages', 'POST', { parent: { database_id: DB.settings }, properties: {
        Name: { title: [{ text: { content: 'ai_spend_month' } }] },
        Value: { rich_text: [{ text: { content: monthKey } }] },
        'Number value': { number: total }
      }});
    }
...
const ACTIONS = {
  ...
  async bootstrap({ date }) {
    ...
    const [habits, logRow, open, review, briefs, sortRow, allSubtasks] = await Promise.all([
      ...
      notion(`databases/${DB.briefs}/query`, 'POST', {
        sorts: [{ property: 'Day', direction: 'descending' }], page_size: 5
      }),
      notion(`databases/${DB.settings}/query`, 'POST', {
        filter: { property: 'Name', title: { equals: 'task_sort' } }, page_size: 1
      }).catch(() => ({ results: [] })),
      // Every subtask regardless of status — an open-only count can't show
      // real progress, since a done subtask disappears from the 'Open'
      // query the instant it's completed. This is the one place that
      // matters, so it's worth the extra query.
      notion(`databases/${DB.tasks}/query`, 'POST', {
        filter: { property: 'Parent task', relation: { is_not_empty: true } },
        page_size: 100
      }).catch(() => ({ results: [] }))
    ]);
    ...
    allSubtasks.results.forEach(p => {
      const parentId = (((p.properties['Parent task'] || {}).relation || [])[0] || {}).id;
      if (!parentId) return;
      const status = sel(p.properties.Status);
      if (!subtaskProgress[parentId]) subtaskProgress[parentId] = { done: 0, total: 0 };
      subtaskProgress[parentId].total++;
      if (status === 'Done') subtaskProgress[parentId].done++;
    });
    ...
    const task = p => ({
      ...
      projectId: (((p.properties['Project link'] || {}).relation || [])[0] || {}).id || '',
  ...
  async categories() {
    ...
    const multiOpts = (db, propName) => (((db.properties || {})[propName] || {}).multi_select || {}).options || [];
    ...
    return {
      ...
      refTags: names(multiOpts(ref, 'Tags'))
    };
  },
  ...
  async search({ q, category }) {
    ...
    const toResult = (p, kindOverride) => {
      ...
      // Sensitive Info records must never leak their value through search,
      // regardless of unlock state elsewhere — this path had no awareness
      // of the Sensitive flag at all until now.
      const sensitive = kind === 'info' && !!(props.Sensitive || {}).checkbox;
      const extra = sensitive
        ? '\u{1F512} Sensitive \u2014 open Info to view'
        : (txt(props.Summary) || txt(props.Value) || txt(props.Headline)
      ...
      return {
        ...
        tags: msel(props.Tags),
        url: p.url || '', edited: p.last_edited_time || '',
        image: sensitive ? '' : firstImageUrl(props.Files)
      };
    };
  ...
  async addRef({ name, topic, tags, summary, source, fileName, fileType, fileData }) {
    ...
    if (Array.isArray(tags) && tags.length) {
      props.Tags = { multi_select: tags.slice(0, 10).map(t => ({ name: String(t).slice(0, 40) })) };
    }
   ...
   */
  /*
   * Recent briefs on Log only show the last 3 by default now — this reaches
   * the rest, with a direct title/headline filter rather than Notion's own
   * global search, since a straight database query is more reliable for
   * exact title matching than fuzzy full-text search.
   */
  async searchBriefs({ q }) {
    const query = String(q || '').trim();
    if (query.length < 2) return { briefs: [] };
    const r = await notion(`databases/${DB.briefs}/query`, 'POST', {
      filter: { or: [
        { property: 'Name', title: { contains: query } },
        { property: 'Headline', rich_text: { contains: query } }
      ]},
      sorts: [{ property: 'Day', direction: 'descending' }],
      page_size: 20
    });
    return {
      briefs: r.results.map(p => ({
        id: p.id, name: txt(p.properties.Name),
        headline: txt(p.properties.Headline), url: p.url, day: dat(p.properties.Day)
      }))
    };
  ...
  /*
   * Drafts a short follow-up for a Waiting On item. Not free — nothing on
   * the Anthropic API is — but genuinely tiny: a few hundred tokens either
   * way, well under a cent per draft. Tracked through the same spend
   * counter the brief itself uses, so it's never a surprise.
   */
  async draftFollowUp({ who, subject, days }) {
    if (!process.env.ANTHROPIC_KEY) throw new Error('ANTHROPIC_KEY is not set on the server');
    const prompt = 'Write a short, polite follow-up message to ' + (who || 'someone')
      + ' about "' + (subject || 'this') + '" — it has been ' + (days || 'a few')
      + ' days since the last reply. Two or three sentences, no greeting or signature, '
      + 'just the body text, ready to paste into an email or message.';
    ...
    const cost = (usage.input_tokens || 0) / 1e6 * 2 + (usage.output_tokens || 0) / 1e6 * 10;
    const monthTotal = await trackAppSpend(cost);
    return { text, cost, monthTotal };
  },

  async projects() {
    const [r, linkedTasks] = await Promise.all([
      notion(`databases/${DB.projects}/query`, 'POST', {
        filter: { or: [
      ...
      notion(`databases/${DB.tasks}/query`, 'POST', {
        filter: { property: 'Project link', relation: { is_not_empty: true } },
        page_size: 100
      }).catch(() => ({ results: [] }))
    ]);

    const progress = {};
    linkedTasks.results.forEach(p => {
      const projectId = (((p.properties['Project link'] || {}).relation || [])[0] || {}).id;
      if (!projectId) return;
      const status = sel(p.properties.Status);
      if (!progress[projectId]) progress[projectId] = { done: 0, total: 0 };
      progress[projectId].total++;
      if (status === 'Done') progress[projectId].done++;
    });
  ...
  async addTask({ name, project, parent, priority, due, projectId, repeat }) {
    const props = {
      Name:   { title: [{ text: { content: String(name).slice(0, 190) } }] },
    ...
    if (projectId) props['Project link'] = { relation: [{ id: projectId }] };
    if (parent) props['Parent task'] = { relation: [{ id: parent }] };
    if (priority) props.Priority = { select: { name: priority } };
    if (due) props.Due = { date: { start: due } };
    if (repeat) props.Repeat = { select: { name: repeat } };
  ...
  async setStatus({ id, status }) {
    if (!['Open', 'Waiting', 'Blocked', 'Done', 'Needs review'].includes(status)) throw new Error('bad status');
    const props = { Status: { select: { name: status } } };
    if (status === 'Done') {
      props['Done at'] = { date: { start: new Date().toISOString() } };
      // Recurring task — spin up the next occurrence before marking this one
      // done. A failure here never blocks completing the current task; it
      // just means no follow-up got created this time.
      try {
        const page = await notion('pages/' + id);
        const repeat = sel(page.properties.Repeat);
        if (repeat) {
          const newProps = {
            Name: { title: [{ text: { content: txt(page.properties.Name).slice(0, 190) } }] },
            Status: { select: { name: 'Open' } },
            Due: { date: { start: nextDueDate(dat(page.properties.Due), repeat) } },
            Repeat: { select: { name: repeat } }
          };
          const projectText = txt(page.properties.Project);
          if (projectText) newProps.Project = { rich_text: [{ text: { content: projectText } }] };
          const projLink = ((page.properties['Project link'] || {}).relation || [])[0];
          if (projLink) newProps['Project link'] = { relation: [{ id: projLink.id }] };
          const priority = sel(page.properties.Priority);
          if (priority) newProps.Priority = { select: { name: priority } };
          await notion('pages', 'POST', { parent: { database_id: DB.tasks }, properties: newProps });
        }
      } catch (e) { /* not fatal — see comment above */ }
    }
    await notion('pages/' + id, 'PATCH', { properties: props });
    return { ok: true };
  },
