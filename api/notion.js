...
const ACTIONS = {
  ...
  async search({ q }) {
    ...
    // /v1/search is a global endpoint, not scoped to a specific database, so
    // it isn't one of the endpoints Notion's 2025-09-03 database→data-source
    // migration affects — using the newer version here (needed to reliably
    // read back a Files property populated via file_upload) shouldn't put
    // this call at the same risk the shared default version is kept away
    // from elsewhere in this file. Worth confirming once deployed.
  ...
  async addRef({ name, topic, summary, source, fileName, fileType, fileData }) {
    ...
    // File uploads are a newer Notion API surface than everything else in
    // this file. Scoping the newer version to just these three calls, rather
    // than bumping the shared default, avoids touching the database-query
    // endpoints elsewhere that would need a bigger migration under it.
  ...
  async addInfo({ name, category, value, notes, sensitive }) {
    if (!name) throw new Error('needs a name');
    const props = { Name: { title: [{ text: { content: String(name).slice(0, 120) } }] } };
